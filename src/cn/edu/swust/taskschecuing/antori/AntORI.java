...
import cn.edu.swust.taskscheduingmethed.MCT;
  ...
  *
  */
 public class AntORI implements Cloneable {
...
private float alpha; 
    ...
    * @param num 蚂蚁数量
    */
   public AntORI(int num,int vmNum){
     taskNum = num;
     this.vmNum=vmNum;
     timecostTotle = 0;
   ...
   public void init(Float[][] etc, float a, float b){
    ...
    int taskseq =ThreadLocalRandom.current().nextInt(taskNum);//随机选择一个任务 //选定一个虚拟机    
   ...
   public void selectNextTask(float[][] pheromone){
     ...
     for (Integer i:allowedTask) {
    	 for(int j=0;j<vmNum;j++){
    	     //sum += Math.pow(pheromone[i][j], alpha)*Math.pow(1.0/(etc[i][j]+costVM[j]), beta);
    		 sum += Math.pow(pheromone[i][j], alpha)*Math.pow(1.0/(etc[i][j]), beta);
    	 }
     }
     //计算概率矩阵
         ...
         if (i == j.intValue()) {
        	 for(int k=0;k<vmNum;k++){
        		// p[i][k] = (float) (Math.pow(pheromone[i][k], alpha)*Math.pow(1.0/(etc[i][k]+costVM[k]), beta))/sum;   
        		 p[i][k] = (float) (Math.pow(pheromone[i][k], alpha)*Math.pow(1.0/(etc[i][k]), beta))/sum; 
      	   } 
        	 flag = true;
             break;
         }
